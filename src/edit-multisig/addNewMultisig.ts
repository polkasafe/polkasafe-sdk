...
import { AddNewMultisigToProxyType, EditMultisigResponse, TransactionCallBack } from './types';

export const addNewMultisigToProxy = async ({
	otherSignatories,
	oldThreshold,
	network,
	api,
	...
	senderAddress,
	proxyAddress
}: AddNewMultisigToProxyType) => {
	let blockHash = '';
	return new Promise<EditMultisigResponse>((resolve, reject) => {

		api.tx.multisig
			.asMulti(oldThreshold, otherSignatories, null, proxyTx, weight as any)
